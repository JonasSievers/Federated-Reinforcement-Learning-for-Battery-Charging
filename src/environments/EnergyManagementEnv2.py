...
class EnergyManagementEnv2(py_environment.PyEnvironment):
    ...
    def __init__(
        ...
        self._observation_spec = array_spec.BoundedArraySpec(shape=(58,), dtype=np.float32, name='observation')
        self._data = data #Data: load, PV, price, emissions
    ...
    def _reset(self):
        ...
        emission_forecast = self._data.iloc[self._current_timestep+1 : self._current_timestep+19, 3]
        prosumption_forecast = load_forecast - pv_forecast
        

         # Add noise to forecast
        if tf.reduce_any(self._noise != 0):
            #pv_forecast = pv_forecast + self._noise.numpy()
            #load_forecast = load_forecast + self._noise.numpy()
            #price_forecast = price_forecast + self._noise.numpy()
            prosumption_forecast = prosumption_forecast + self._noise.numpy()
            emission_forecast = emission_forecast + self._noise.numpy()
        ...
        observation = np.concatenate(([self._soe, p_prosumption, grid_emissions, electricity_price], prosumption_forecast, emission_forecast, price_forecast), dtype=np.float32)
    ...
    def _step(self, action):
        ...
        penalty_soe  = np.abs(action[0] - (self._soe - soe_old))

        #1.2 Battery aging
        lower_threshold, upper_threshold = 0.10 * self._capacity, 0.90 * self._capacity
        if self._soe < lower_threshold:
            penalty_aging = (lower_threshold - self._soe)
        elif self._soe > upper_threshold:
            penalty_aging = (self._soe - upper_threshold)
        else:
            penalty_aging = 0 

        penalty_impact = penalty_soe + penalty_aging
        ...
        profit_impact = ((profit - cost))*(1-self._ecoPriority)
        ...
        emissions_impact = emissions*self._ecoPriority
        

        reward = profit_impact*5 - emissions_impact - penalty_impact
        ...
        if self._logging:
            ...
            'Emissions impact [kg]': emissions_impact,
            'Reward' : reward,
            'PV': p_pv, 
            'Load' : p_load, 
            'Price' : electricity_price,
            'Net load': p_prosumption,
            })
